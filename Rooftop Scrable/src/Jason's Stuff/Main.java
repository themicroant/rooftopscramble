...
public class Main extends Applet implements Runnable
{
	...
	public void init()
	{
		...
		PS0 = Transparency.makeColorTransparent(getImage(getDocumentBase(),
				"megagirl_standing.gif"), new Color(0).white);
		PS1 = Transparency.makeColorTransparent(getImage(getDocumentBase(),
				"megagirl_running.gif"), new Color(0).white);
		PS2 = Transparency.makeColorTransparent(getImage(getDocumentBase(),
				"megagirl_jumping.gif"), new Color(0).white);

		buildingA = new Building(0, 410);
		buildingA.setImage(Transparency.makeColorTransparent(getImage(
				getDocumentBase(), "rooftop.gif"), new Color(0).black));
		EArray.add(buildingA);

		player = new Player(width / 2, 200);
		player.setImage(Transparency
				.makeColorTransparent(getImage(getDocumentBase(),
						"megagirl_standing.gif"), new Color(0).white));
		EArray.add(player);
		...
		for (int i = 0; i < 1; i++) {
			...
			enemy[i].setImage(Transparency.makeColorTransparent(getImage(
					getDocumentBase(), "my_bird.gif"), new Color(0).white));
			EArray.add(enemy[i]);
		}
		shots = new Shot[5];
